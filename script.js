...
// 1) Estructura completa de la malla con prerrequisitos
const materias = [
  {
    ...
    anio: "Semestre 3", semestres: [
      {
        numero: "3º Semestre", materias: [
          { id: "s3_1", nombre: "Álgebra Lineal", previas: ["s2_1", "s2_2"] },
          { id: "s3_2", nombre: "Cálculo II", previas: ["s2_1", "s2_2"] },
    ...
    anio: "Semestre 5", semestres: [
      {
        numero: "5º Semestre", materias: [
          ...
          { id: "s5_2", nombre: "Mecánica de Fluidos", previas: ["s4_2"] },
          { id: "s5_3", nombre: "Optimización", previas: ["s3_1", "s4_1"] },
    ...
    anio: "Semestre 6", semestres: [
      {
        numero: "6º Semestre", materias: [
          { id: "s6_1", nombre: "Electricidad y Electromagnetismo", previas: ["s3_3"] },
          { id: "s6_2", nombre: "Termodinámica", previas: ["s5_1"] },
          { id: "s6_3", nombre: "Investigación Operativa", previas: ["s5_3"] },
    ...
    anio: "Semestre 7", semestres: [
      {
        numero: "7º Semestre", materias: [
          ...
          { id: "s7_2", nombre: "Gestión de Operaciones I", previas: ["s6_3"] },
          { id: "s7_3", nombre: "Especialidad I", previas: [] },
          { id: "s7_4", nombre: "Macroeconomía", previas: ["s6_4"] },
          { id: "s7_5", nombre: "Ingeniería Económica", previas: ["s6_3"] },
    ...
    anio: "Semestre 8", semestres: [
      {
        numero: "8º Semestre", materias: [
          ...
          { id: "s8_4", nombre: "E-Commerce & E-Business", previas: ["s2_4"] },
          { id: "s8_5", nombre: "Finanzas", previas: ["s5_5"] },
    ...
    anio: "Semestre 9", semestres: [
      {
        numero: "9º Semestre", materias: [
          { id: "s9_1", nombre: "Procesos Industriales", previas: ["s4_4"] },
          { id: "s9_2", nombre: "Especialidad II", previas: ["s7_3"] },
          { id: "s9_3", nombre: "Innovación y Emprendimiento", previas: ["s7_5"] },
          { id: "s9_4", nombre: "Formulación y Evaluación de Proyectos", previas: ["s7_5"] },
          { id: "s9_5", nombre: "Economía y Desarrollo Regional", previas: ["s7_4"] },
    ...
    anio: "Semestre 10", semestres: [
      {
        numero: "10º Semestre", materias: [
          { id: "s10_1", nombre: "Simulación de Procesos", previas: ["s5_3"] },
          { id: "s10_2", nombre: "Derecho de Empresa y Ambiental", previas: [] },
          { id: "s10_3", nombre: "Marketing", previas: [] },
          { id: "s10_4", nombre: "Administración y Dirección de Proyectos", previas: ["s8_1"] },
    ...
    anio: "Semestre 11", semestres: [
      {
        numero: "11º Semestre", materias: [
          { id: "s11_1", nombre: "Especialidad III", previas: ["s9_2"] },
          { id: "s11_2", nombre: "Trabajo de Título", previas: ["s10_5"] }
        ]
      }
    ]
  }
...
// 2) Carga/Guarda en localStorage
const estadoMaterias = JSON.parse(localStorage.getItem("estadoMaterias") || "{}");

// 3) Función de renderizado con bloqueo/desbloqueo
function crearMalla() {
  ...
  materias.forEach(anio => {
    ...
    anio.semestres.forEach(sem => {
      ...
      sem.materias.forEach(mat => {
        ...
        divMat.textContent = mat.nombre;
        divMat.className = "materia";
        divMat.dataset.id = mat.id;

        // Aplica tachado si ya cursada
        if (estadoMaterias[mat.id]) {
          divMat.classList.add("tachada");
        }

        // Bloquea si no cumple prerrequisitos
        const okPrevias = mat.previas.every(pid => estadoMaterias[pid]);
        if (!okPrevias) {
          divMat.classList.add("bloqueada");
        }

        // Toggle al click y re-renderizado para actualizar bloqueos
        divMat.addEventListener("click", () => {
          if (divMat.classList.contains("bloqueada")) return;
          divMat.classList.toggle("tachada");
          estadoMaterias[mat.id] = divMat.classList.contains("tachada");
